...
#include: "20Y1JR5RLL*.dashboard.lookml"
...
explore: fact_transaction {
  ...
  sql_always_where: ${vw_dim_company.company_code} = '20Y1JR5RLL' ;;

  join: 20Y1JR5RLL_vw_dim_user_custom_field {
    type: left_outer
    sql_on:  ${vw_fact_transaction_entry.user_custom_field_key} = ${20Y1JR5RLL_vw_dim_user_custom_field.user_custom_field_key}  ;;
    relationship: many_to_one
    #fields:[]
  }
  join: 20Y1JR5RLL_vw_dim_user_profile_field {
    type: left_outer
    sql_on:  ${vw_fact_transaction_entry.user_profile_field_key} = ${20Y1JR5RLLvw_dim_user_profile_field.user_profile_field_key}  ;;
    relationship: many_to_one
    #fields: []
  }
