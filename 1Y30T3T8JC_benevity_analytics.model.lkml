...
#include: "1Y30T3T8JC*.dashboard.lookml"
...
explore: fact_transaction {
  ...
  sql_always_where: ${vw_dim_company.company_code} = '1Y30T3T8JC' ;;

  join: 1Y30T3T8JC_vw_dim_user_custom_field {
    type: left_outer
    sql_on:  ${vw_fact_transaction_entry.user_custom_field_key} = ${1Y30T3T8JC_vw_dim_user_custom_field.user_custom_field_key}  ;;
    relationship: many_to_one
    #fields:[]
  }
  join: 1Y30T3T8JC_vw_dim_user_profile_field {
    type: left_outer
    sql_on:  ${vw_fact_transaction_entry.user_profile_field_key} = ${1Y30T3T8JCvw_dim_user_profile_field.user_profile_field_key}  ;;
    relationship: many_to_one
    #fields: []
  }
