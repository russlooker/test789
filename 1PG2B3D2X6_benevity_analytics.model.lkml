...
#include: "1PG2B3D2X6*.dashboard.lookml"
...
explore: fact_transaction {
  ...
  sql_always_where: ${vw_dim_company.company_code} = '1PG2B3D2X6' ;;

  join: 1PG2B3D2X6_vw_dim_user_custom_field {
    type: left_outer
    sql_on:  ${vw_fact_transaction_entry.user_custom_field_key} = ${1PG2B3D2X6_vw_dim_user_custom_field.user_custom_field_key}  ;;
    relationship: many_to_one
    #fields:[]
  }
  join: 1PG2B3D2X6_vw_dim_user_profile_field {
    type: left_outer
    sql_on:  ${vw_fact_transaction_entry.user_profile_field_key} = ${1PG2B3D2X6vw_dim_user_profile_field.user_profile_field_key}  ;;
    relationship: many_to_one
    #fields: []
  }
